...
const Footer = () => {
  ...
  return (
    ...
    <div className={styles.recentPost} style={{ position: "relative" }}>
      ...
      {data.slice(0,3).map((article, index) => (
        ...
        <li key={article.guid} className={styles.post}>
          ...
          <div  >
          {article.thumbnail && (
              <img src={article.thumbnail} className={styles.postImage}  alt={article.title} />
            )}
          </div>
          <div className={styles.postContent}>
          <div className={styles.postTitle}><h4><a href={article.link}>{article.title}</a></h4> </div>
